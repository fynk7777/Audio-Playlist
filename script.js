...
if (window.location.protocol === "https:") {
    ...
    async function getAudioOutputDevices() {
        try {
            ...
            // 現在選択されているデバイスがリスト内にない場合は、最初のデバイスを選択
            if (!Array.from(outputDevices.options).some(option => option.value === outputDevices.value)) {
                outputDevices.value = audioOutputs.length > 0 ? audioOutputs[0].deviceId : '';
            }
